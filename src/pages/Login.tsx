...
const Login = () => {
  ...
  const createOrUpdateUserRecord = async (userId: string, user: any) => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        // User doesn't exist, create new user record with full info
        await set(userRef, {
          email: user.email,
      ...
      // We don't want to block the login process if this fails
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (userCredential?.user?.uid) {
        ...
        await createOrUpdateUserRecord(userCredential.user.uid, userCredential.user);
      }
      ...
      if (error instanceof FirebaseError) {
        ...
        if (error instanceof Error && error.message.includes('@vitstudent.ac.in')) {
          toast.error('Only VIT Student email ID is allowed');
        } else {
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
            <pre className="font-mono text-terminal-highlight animate-pulse">
          _  ____                 __{'\n'}
          ...
          <p className="mt-2 text-white-600 dark:text-white-400">
